...
- _updated!_ `feather` (as oppopsed to `arrow-parquet`) is now the clear winner for read speeds! The next best is `arrow-parquet` at 2.5x slower. `feather` is simply _blazing!_ At sub-second read speeds, I had to confirm something wasn't going wrong in the code-base!
