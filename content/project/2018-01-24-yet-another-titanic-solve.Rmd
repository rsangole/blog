...
## Extreme Gradient Boosting - Reduced Variables
...
xgbFit.red <- train(
    Survived~.,
    train.imp %>% dplyr::select(Survived, xgb.importance(feature_names = colnames(train.imp), model = xgbFit$finalModel)$Feature),
    method = 'xgbTree',
    trControl = ctrl,
    tuneGrid = xgbGrid
)
save(xgbFit.red,file = 'xgbFit.red')
xgbFit.red
plot(xgbFit.red)
densityplot(xgbFit.red,pch='|')
predict(xgbFit.red,type = 'prob') -> train.Probs
histogram(~Survived+Dead,train.Probs)
```
...
plot(varImp(xgbFit.red),15,main='XGBoost, Reduced Variables')
```
...
xgbPred_red <- predict(object = xgbFit.red, newdata = test.imp)
...
xtab <- table(xgbPred_red,test.imp$Survived)
xgbredCM <- caret::confusionMatrix(xtab)
...
CM_list <- list(xgbCM, xgbredCM, elastinetCM, c5CM, knnCM, svmCM, avNNCM)

compiled_results <- tibble(
    models = c('xgb','xgb_reduced','elastinet','C5.0','knn','svm', 'avgNN'),
...
```{r}
dotplot(reorder(models,accuracy)~accuracy,compiled_results)
xyplot(kappa~F1,compiled_results,
       panel = function(...){
           panel.xyplot(...)
           ltext(...,labels=compiled_results$models,
                 pos=1, cex=.8, offset = .6,col='darkblue')
       })
```
