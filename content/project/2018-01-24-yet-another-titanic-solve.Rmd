...
xgb.red.ROC <- roc(response = xgbFit.red$pred$obs, predictor = xgbFit.red$pred$Survived)
xgbFit.red
plot(xgbFit.red)
...
# glmnetROC <- roc(response = glmnetFit$pred$obs, predictor = glmnetFit$pred$Survived)
glmnetFit
...
svmFit <- train(
    ...
    method = 'svmRadial',
    trControl = ctrl,
    tuneGrid = expand.grid(C=c(0.5,0.8,1.2,1.5), sigma=c(0.05,0.1,0.2))
)
